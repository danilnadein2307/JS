...
document.addEventListener("DOMContentLoaded", async function() {
  ...
  function addOneStudent(studentObj) {
    ...
    const nameStringArr = studentName.textContent.trim().split(" ");
    const joinedName = nameStringArr.join("");
    studentObj['fullname'] = joinedName;
    ...
    studentObj['endyear'] = Number(studentObj.studyStart) + 4;
  ...
  function createStudentsTable(arr) {
    ...
    let newArr = [...arr];
    if (nameValue !== "") newArr = filter(newArr, "fullname", nameValue);
    if (facultyValue !== "")
      newArr = filter(newArr, "faculty", facultyValue.toLowerCase());
    if (startYearValue !== "")
      newArr = filter(newArr, "studyStart", startYearValue);
    if (endYearValue !== "") newArr = filter(newArr, "endyear", endYearValue);
