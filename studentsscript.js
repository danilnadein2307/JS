...
document.addEventListener("DOMContentLoaded", async function() {
  ...
  // В следующем ДЗ лучше использовать универсальную функцию для создания элементов: кода меньше, читабельность выше, писать проще
  let table = document.createElement("table");
  table.classList.add("table", "table-success", "table-striped");
  document.body.append(table);
    // Здесь обращение к DOM (что является самой медленной операцией на фронте), потому что элементы уже вставлены в body
  ...
  // let везде лучше заменить на const, чтобы случайно не переопределить переменную (почти во всех случаях здесь не используется переопределение)
  ...
  // max нужно высчитывать от актуальной даты
  ...
  // Отступы поехали, в некоторых местах отсутствует точка с запятой, писать надо в одном стиле для улучшения читабельности
  ...
  async function getStudents() {
    // Не знаю, проходил ты или нет, но такое нужно оборачивать в try-catch, иначе ошибка в запросе (например если сервер не ответил) кладет все приложение (открывается просто белая страница)
    const response = await fetch('http://localhost:3000/api/students')
  ...
  function validation(form) {
    ...
    for (const input of allInputs) {
      removeError(input);
      // Здесь идет сравнение ссылок для объекты (разберем на созвоне, если непонятно). Если элемент перерендерится - ссылка поменяется, проверка сломается. Нужно добавить уникальный id для инпутов и сравнивать его
      if (input == nameInput) {
        // Логика следующих четырех строк используется несколько раз, только с другим input и текстом ошибки, такое нужно выносить в функции
      ...
      // Сравнение всегда нужно использовать строгое и делать явное приведение типов, где требуется. Нестрогое сравнение часто выдает не ожидаемое поведение, что потом трудно продебажить
      ...
      if (input == studyYearInput) {
        if (studyYearInput.value.trim() == "") {
          ...
          // 2000 и 2024 - magic number, незнакомый с логикой человек и автор кода со временем забывает что это за значения. Поэтому такое нужно выносить в константы с подробным понятным названием (и желательно комментарием)
  ...
  form.addEventListener("submit", function(event) {
    event.preventDefault();
    // сравнение не имеет смысла, потому что validation возвращает true или false, можно просто подставить результат вызова функции в if -  if (validation(this))
    if (validation(this) == true) {
      let nameStringArr = nameInput.value.trim().split(" ");
      let joinedName = nameStringArr.join("");
      // Если не будет трех значений разделенных пробелом, выпадет ошибка в консоль, нужно учитывать
      const newStudent = {
        surname:
...
createStd()
  ...
  function dateOfBirth(date) {
    return (
      // Не понял, зачем здесь оператор побитового или - |, обсудим на созвоне
  ...
  function sort(button, arr, prop) {
    button.addEventListener("click", function() {
      console.log('call')
      sortUsers(arr, prop);
      // reverse не нужен, функция sortUsers может сортировать в двух направлениях в зависимости от возврата -1 или 1. Можем разобрать на созвоне, логика как в доп.видосе, только адекватно написанная
  ...
  // Эта функция при каждом вызове вешает новый слушатель событий на кнопке, что приводит к увеличению количества вызовов функции-слушателя при каждом клике и утечке памяти, разберем
  ...
  // Функция sort ничего не возвращает, в переменных хранится undefined
